...
export const BuildUserShare = (user) => {
    ...
    return <div className={'personBlock'}>
        ...
        <p className={'sharePrice'}>Total share - <span>${parseFloat(user.share.toFixed(2))}</span></p>
        {user.items.length && !showEmail ?<button className={'sendEmail'} onClick={() => {
            setShowEmail(true)
        }}>Send Email
        </button>:''}

        {showEmail && <div>
            <input className={'emailInput'} placeholder={'enter email'} onChange={(e) => {
                e.preventDefault();
                setEmail(e.target.value)
            }} value={email} type="email"/>
            <button className={'sendEmail'} onClick={sendEmail}>{'Send'}</button>
            <button className={'sendEmail'} style={{width:`60px`}} onClick={() => {
                setShowEmail(false)
            }}>{'X'}</button>
        </div>}
